...
let encore env { dv = dv } =
  let write_on_last x =
    let { t_sort = sort } =
      try Env.find x env
      with Not_found ->
	Zmisc.internal_error "Encore: unbound name" Printer.name x in
      match sort with
...
let with_zero env encore_opt ({ b_body = eq_list; b_write = w } as b) =
  if encore env w then
	...
	| None -> Zident.fresh "encore" | Some(encore) -> encore in
...
and equation_list env encore_opt eq_list =
  Zmisc.map_fold (equation env) encore_opt eq_list      
