...
let rec pattern dv copies pat =
  ...
  | Etuplepat(p_list) ->
      let p_list, copies = Zmisc.map_fold (pattern dv) copies p_list in
  ...
  | Evarpat(n) -> 
      if S.mem n dv then
        let ncopy = Zident.fresh "copy" in
...
and equation_list dv copies eq_list = 
  let eq_list, copies_eq_list = Zmisc.map_fold (equation dv) Env.empty eq_list in
